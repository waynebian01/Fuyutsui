...
1. 用途限制：本软件（Fuyutsui）仅供技术研究、学术交流及个人学习使用。用户在使用本软件时，必须遵守当地法律法规以及相关服务提供商（如游戏运营商）的用户协议。

2. 风险自担：用户明确了解并同意，使用本软件可能存在被第三方平台（如游戏反作弊系统）检测并导致账号受限、封禁或其他处罚的风险。开发者不对任何因使用本软件而导致的账号损失、数据丢失或法律纠纷承担责任。

3. 无担保性：本软件按“原样”（As Is）提供，开发者不保证软件功能的稳定性、完整性或兼容性，亦不承担因软件 Bug 造成的任何直接或间接损失。

4. 非商业化：除非另有书面授权，本软件严禁用于任何形式的商业营利行为（包括但不限于有偿代练、打包销售等）。
